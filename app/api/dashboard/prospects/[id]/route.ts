...
/**
 * PATCH /api/dashboard/prospects/[id]
 * Mettre à jour UN prospect (vérifier qu'il appartient à l'entreprise)
 */
export async function PATCH(
  ...
  try {
    ...
    console.error('Error in PATCH /api/dashboard/prospects/[id]:', error)
...
/**
 * DELETE /api/dashboard/prospects/[id]
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error in DELETE /api/dashboard/prospects/[id]:', error)
    const message = error instanceof Error ? error.message : 'Erreur serveur'
    return NextResponse.json(
      { error: message },
      { status: message.includes('authentifié') ? 401 : 500 }
