...
export default function SettingsPage() {
    ...
    const initPage = async () => {
      try {
        ...
        if (session?.user?.email) {
          ...
          await loadClientData(session.user.id, session.user.email)
        } else {
          ...
          const { data: { subscription } } = supabase.auth.onAuthStateChange(
            async (_event, newSession) => {
              ...
              if (newSession?.user?.email) {
                ...
                await loadClientData(newSession.user.id, newSession.user.email)
              } else {
  ...
  const loadClientData = async (userId: string, userEmail: string) => {
    try {
      console.log('📥 Chargement des données pour:', userEmail)

      // Récupère les données du client
      // Grâce à la RLS, on ne verra que nos propres données
      ...
      console.log('✅ Données chargées:', client)
    ...
    try {
      ...
      if (userData.client) {
        ...
        const { error: updateError } = await supabase
          ...
          .eq('id', userData.client.id)

        if (updateError) throw updateError
      } else {
        // Création - IMPORTANT: ajouter created_by pour la sécurité RLS
